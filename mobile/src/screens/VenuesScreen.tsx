...
const VenuesScreen = () => {
    ...
    // Get user's city and favorite sports from profile
    const userCity = (user as any)?.city || 'Hyderabad';
    const userSports = (user as any)?.favorite_sports || [];
    ...
    const fetchVenues = async () => {
        setIsLoading(true);
        try {
            // Filter by user's city and selected game type (or all user's sports)
            const gameTypeFilter = selectedGameType ? selectedGameType : userSports;

            const response = await venuesApi.getVenues({
                location: userCity,
                game_type: gameTypeFilter.length > 0 ? gameTypeFilter : undefined,
            });
    ...
    const getGameTypes = (venue: Venue): string[] => {
        if (Array.isArray(venue.game_type)) {
            return venue.game_type;
        }
        ...
        <SafeAreaView style={styles.container} edges={['top']}>
            ...
            <View style={styles.header}>
                ...
                <View style={styles.headerCenter}>
                    <Text style={styles.headerTitle}>Venues in {userCity}</Text>
            ...
            {userSports.length > 0 && (
                ...
                >
                    <TouchableOpacity
                        style={[styles.filterChip, !selectedGameType && styles.filterChipActive]}
                        onPress={() => setSelectedGameType(null)}
                    >
                        <Text style={[styles.filterChipText, !selectedGameType && styles.filterChipTextActive]}>
                            All
                        </Text>
                    </TouchableOpacity>
                    {userSports.map((sport: string) => (
                        <TouchableOpacity
                            key={sport}
                            style={[styles.filterChip, selectedGameType === sport && styles.filterChipActive]}
                            onPress={() => setSelectedGameType(sport)}
                        >
                            <Text style={[styles.filterChipText, selectedGameType === sport && styles.filterChipTextActive]}>
                                {sport}
                            </Text>
                        </TouchableOpacity>
                    ))}
